...
class FCMDeviceQuerySet(models.query.QuerySet):
	def send_message(self, title, message, **kwargs):
		if self:
			...
			return fcm_send_bulk_message(registration_ids=reg_ids, title=title, message=message, **kwargs)
...
class FCMDevice(Device):
	...
	def send_message(self, title, message, **kwargs):
		...
		return fcm_send_message(registration_id=self.registration_id, title=title, message=message, **kwargs)
